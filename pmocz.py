...
from kernels import dwdq_gauss#, w_gauss
...
def getAcc( pos, vel, m, h, k, n, lmbda, nu ):
    ...
    radius = np.sqrt(dx**2 + dy**2 + dz**2).astype('f4')
    dwdq = dwdq_gauss(radius, h, 3)
    grad_w = np.zeros(radius.shape)
...
def main():
    ...
    pos   = np.reshape(np.random.randn(32,32,3), (32*32,3))   # randomly selected positions and velocities
