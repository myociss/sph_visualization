...
np_path = os.path.join(os.path.dirname(__file__), f'data/jupiter_relaxed_3d_pos_128.npy')
with open(np_path,'rb') as f:
    pos_init = np.load(f)

np_path = os.path.join(os.path.dirname(__file__), f'data/jupiter_relaxed_3d_pos_small_128.npy')
with open(np_path,'rb') as f:
    pos_init_small = np.load(f)


np_path = os.path.join(os.path.dirname(__file__), f'data/jupiter_relaxed_3d_energy_128.npy')
with open(np_path,'rb') as f:
    energy_init = np.load(f)

np_path = os.path.join(os.path.dirname(__file__), f'data/jupiter_relaxed_3d_energy_small_128.npy')
with open(np_path,'rb') as f:
...
np_path = os.path.join(os.path.dirname(__file__), f'data/jupiter_relaxed_3d_mass_128.npy')
with open(np_path,'rb') as f:
    mass = np.load(f)
...
h_init = 0.07 * R#works
#h_init = 0.1 * R * 1.1#doesn't work
#h_init = 0.07 * R#doesn't work
...
all_pos = np.zeros((particle_dim,particle_dim,spatial_dim,steps), dtype='f4')
...
for i in range(steps):
    ...
    all_pos[:,:,:,i] = d_pos.copy_to_host()
    
    all_rho[:,i] = (d_rho.copy_to_host()).reshape((N,))

    #with open(f'data/collision_frames/frame_{i}.npy', 'wb') as f:
    #    np.save(f, all_pos[:,:,i])
    ...
    #if i % 3 == 0:
    imgs.append(get_img_alt(all_pos[:,:,:,i].reshape((N,spatial_dim)), all_rho[:,i], R*4))
...
with open(f'data/collision_128_all_frames.npy', 'wb') as f:
    np.save(f, all_pos)
